...
const resolveExtensions = (dirPath: string) => (context: ts.TransformationContext) => {
    return (rootNode: ts.Node) => {
        function visit(node: ts.Node) {
            ...
            if (ts.isImportDeclaration(node)) {
                ...
                const to = ((node as ts.CallExpression).arguments[0] as ts.StringLiteral).text;
                if (isRelative(to)) {
                    const newLoc = locate(dirPath, to);
                    ((node as ts.CallExpression).arguments[0] as ts.StringLiteral).text = newLoc;
                }
            }
            return node;
        }
...
const updateFileImports = (filePath: string) => {
    if (existsSync(filePath)) {
        ...
        console.log("Please update 'targetFilePath' to point to an existing .ts or .js file.");
    }
};
for (const entry of readdirSync(".", { recursive: true, withFileTypes: true })) {
    ...
    try {
        ...
        console.warn(`${e.message} in ${file}`);
    }
}
